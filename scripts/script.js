...
const loadContainerProducts = (data) => {
        ...
        data.forEach(product => {
                ...
                templateContainerProducts.querySelector('h3').textContent = `${product.name}`;
...
btnNavGatitos.addEventListener('click',(e)=>{
        e.preventDefault();
        let filter = filterProducts('Gatitos');
        if (nav__menu.classList.contains('show')) {
                nav__menu.classList.toggle('show');
                btn__nav.classList.toggle('active');
        }
        productsInformationTitle.textContent=`Categoria: Gatitos`;
...
btnNavGatosAdultos.addEventListener('click', (e) => {
        e.preventDefault();
        let filter = filterProducts('Adultos');
        if (nav__menu.classList.contains('show')) {
                nav__menu.classList.toggle('show');
                btn__nav.classList.toggle('active');
        }
        productsInformationTitle.textContent=`Categoria: Adultos`;
        productsInformationResults.textContent=`${ filter.length} Resultados`
        scrollTo(0,productsInformation.offsetTop - 50);
        setTimeout(() => {
                loadContainerProducts(filter);
...
btnNavLineaPremium.addEventListener('click', (e) => {
        e.preventDefault();
        let filter = filterProducts('Premium');
        if (nav__menu.classList.contains('show')) {
                nav__menu.classList.toggle('show');
                btn__nav.classList.toggle('active');
        }
        productsInformationTitle.textContent=`Categoria: Premium`;
        productsInformationResults.textContent=`${ filter.length} Resultados`
        scrollTo(0,productsInformation.offsetTop - 50);
        setTimeout(() => {
                loadContainerProducts(filter);
...
btnFooterGatitos.addEventListener("click",(e)=>{
        e.preventDefault();
        let filter = filterProducts('Gatitos');
        if (nav__menu.classList.contains('show')) {
                nav__menu.classList.toggle('show');
...
btnFooterAdultos.addEventListener('click', (e)=>{
        e.preventDefault();
        let filter = filterProducts('Adultos');
        if (nav__menu.classList.contains('show')) {
                nav__menu.classList.toggle('show');
...
btnFooterPremium.addEventListener('click', (e) => {
        e.preventDefault();
        let filter = filterProducts('Premium');
        if (nav__menu.classList.contains('show')) {
                nav__menu.classList.toggle('show');
